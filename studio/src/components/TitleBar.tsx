...
export function TitleBar(): React.JSX.Element {
  const { project, theme, setTheme, toggleSidebar, toggleBottom, openProjectWizard } = useWorkbench();
  ...
  return (
    <header className="titlebar">
      ...
      <nav className="menu-strip" aria-label="Application menu">
        <button onClick={openProjectWizard}>File</button><button>Edit</button><button onClick={openProjectWizard}>Project</button><button>Build</button><button>Hardware</button><button>Help</button>
      </nav>
